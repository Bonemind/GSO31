 ...
 * @author Michon
 */
class IPeriod2Test extends GroovyTestCase {
    private static void assertEquals(ITime c1, ITime c2) {
        assertEquals(c1.compareTo(c2), 0)
    }

    void testLength() {
        IPeriod pp = new Period2(new Time(2014, 8, 8, 12, 10), new Time(2014, 8, 9, 10, 34));
    ...
    void testMove() {
        IPeriod pp = new Period2(new Time(2014, 8, 8, 12, 10), new Time(2014, 8, 9, 10, 34));
        pp.move(10);
        IPeriod pm = new Period2(new Time(2014, 8, 8, 12, 20), new Time(2014, 8, 9, 10, 44));
    ...
    void testChangeLengthWith() {
        ...
        IPeriod pm = new Period2(new Time(2014, 8, 8, 12, 10), new Time(2014, 8, 9, 10, 44));
        assertEquals(pm.getBeginTime(), pp.getBeginTime());
        assertEquals(pm.getEndTime(), pp.getEndTime());
    }
    ...
    void testIsPartOf() {
        // Define time periods.
        IPeriod pp = new Period2(new Time(2014, 8, 8, 12, 10), new Time(2014, 8, 9, 10, 34));
        IPeriod pa = new Period2(new Time(2014, 8, 8, 10, 4), pp.getBeginTime());             // Before and until start of pp.
        IPeriod pb = new Period2(new Time(2014, 8, 8, 10, 22), new Time(2014, 8, 8, 14, 45)); // Before and in pp.
        IPeriod pc = new Period2(new Time(2014, 8, 8, 16, 34), new Time(2014, 8, 9, 6, 43));  // Completely inside pp.
        IPeriod pd = new Period2(new Time(2014, 8, 9, 17, 56), new Time(2014, 8, 10, 5, 45)); // No overlap with pp.
        ...
        // Consecutive Period2 at begin.
        assertFalse(pp.isPartOf(pa));
        assertFalse(pa.isPartOf(pp));

        // One Period2 fully contained in the other.
